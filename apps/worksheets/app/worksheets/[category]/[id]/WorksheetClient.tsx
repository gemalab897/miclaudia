...
export default function WorksheetClient({ worksheet, categoryId, categoryTitle, categoryColor, categoryIcon }: Props) {
  ...
  const handleReset = () => {
    if (confirm("Clear all answers in this worksheet?")) {
      setValues({});
      setSavedAt(null);
    ...
    <div className="max-w-3xl mx-auto px-6 py-10">
      ...
      >
        ...
        <div className="mt-5 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white/70 rounded-xl p-4">
            <div className="text-[10px] font-bold uppercase tracking-wide mb-1" style={{ color: categoryColor }}>Goal</div>
            <p className="text-xs text-slate-600 leading-relaxed">{worksheet.goal}</p>
          </div>
          <div className="bg-white/70 rounded-xl p-4">
            <div className="text-[10px] font-bold uppercase tracking-wide mb-1 text-slate-400">Instructions</div>
      ...
      {worksheet.therapistNote && (
        <div className="mb-6 flex gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200 no-print">
          ...
          <div>
            <div className="text-[10px] font-bold uppercase tracking-wide text-amber-600 mb-0.5">Therapist Note</div>
      ...
      <div className="flex items-center justify-between mb-4 no-print">
        <div className="flex items-center gap-1.5 text-xs text-slate-400">
          ...
          {justSaved ? (
            ...
            <span>Last saved at {savedAt}</span>
          ) : (
