...
export const lessons: Lesson[] = [
  ...
  },
  {
    ...
    content: [
      ...
      p("En su libro $100M Offers, Alex Hormozi propone una fórmula simple para entender por qué una oferta se siente irresistible o se siente floja, incluso con el mismo precio: el valor percibido sube cuando aumenta el resultado que la persona desea y la confianza de que lo va a lograr, y baja cuando aumenta el tiempo que le va a tomar verlo o el esfuerzo que siente que le va a exigir."),
      quote("Valor percibido = (Resultado deseado × Probabilidad percibida de lograrlo) ÷ (Tiempo hasta el resultado × Esfuerzo y sacrificio percibido)"),
      p("Adaptada a la ética del gremio, cada palanca de esta ecuación se mueve así: el \"resultado deseado\" no es una promesa clínica, es la claridad con la que comunicas hacia dónde va el proceso (tu PVU de la lección 4). La \"probabilidad percibida\" se construye con prueba social ética y con la claridad de tu método (lecciones 4 y 9). El \"tiempo\" y el \"esfuerzo\" se reducen haciendo que dar el primer paso —agendar, la primera sesión— sea lo más simple y liviano posible. Ninguna de las cuatro palancas requiere bajar el precio; todas se mueven con mejor comunicación y mejor diseño de la experiencia."),
      ...
      h3("De problema a oferta irresistible", "🧱"),
      p("El método de Hormozi para armar el value stack no es enumerar servicios al azar: es tomar cada problema u obstáculo específico que identificaste en la ficha de dolor y objeciones (lección 3) y diseñar un elemento concreto de tu oferta que lo resuelva. La suma de esas soluciones, presentadas juntas, es lo que hace que la oferta se sienta imposible de rechazar — no porque sea barata, sino porque cada objeción ya tiene una respuesta incluida antes de que la persona la plantee."),
      list([
        "Problema: \"no tengo tiempo para agregar terapia a mi semana\" → Incluye: sesiones de 45 minutos con horarios tempranos y nocturnos disponibles.",
        "Problema: \"no sé si esto realmente me va a servir\" → Incluye: primera sesión de evaluación sin cargo si no hay compatibilidad.",
      ]),
  ...
  },
  {
    ...
    toolLabel: "Librería de ganchos y constructor AIDA",
    content: [
      ...
      h3("La técnica AIDA", "🎬"),
      p("AIDA es otra forma de mirar la misma arquitectura, con un paso adicional que conviene distinguir del gancho-desarrollo-CTA: separa el \"desarrollo\" en dos momentos distintos, Interés y Deseo, porque no es lo mismo lograr que alguien siga leyendo que lograr que alguien quiera actuar."),
      list([
        "Atención: la primera línea o los primeros segundos. Cumple la misma función que el gancho — detener el scroll.",
        "Interés: profundiza mostrando que entiendes el problema en detalle, con especificidad que solo alguien que trabaja con ese avatar podría describir.",
        "Deseo: pinta cómo se ve la vida del lado de después — el resultado, no el proceso clínico. Es el momento donde la persona empieza a imaginarse ahí.",
        "Acción: el mismo CTA claro y directo — comentar, guardar, escribir por mensaje o agendar.",
      ]),
      p("AIDA se conecta directo con los niveles de consciencia de la lección anterior: Atención e Interés funcionan mejor con audiencias en los niveles 1 y 2 (todavía no saben que hay solución); Deseo y Acción funcionan mejor con audiencias en los niveles 4 y 5 (ya solo necesitan el empujón final). Usa esta técnica cuando quieras construir un post más largo o una publicación de conversión directa; usa el gancho-desarrollo-CTA cuando quieras algo más corto y ágil."),
      ...
      p("La herramienta de esta lección tiene dos partes: una librería de ganchos filtrable por especialidad y nivel de consciencia, y un constructor de post con la técnica AIDA que arma el texto completo a partir de tus 4 campos."),
    ],
  },
  {
    id: 9,
