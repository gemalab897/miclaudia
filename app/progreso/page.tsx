...
export default function ProgresoPage() {
  ...
  const TABS: { id: Tab; label: string; done: number; total: number; color: string }[] = [
    { id: "casos",      label: "Casos clínicos", done: casosCount,      total: casos.length,      color: "#dc2626" },
    { id: "protocolos", label: "Protocolos",      done: protocolosCount, total: protocolos.length, color: "#7c3aed" },
    { id: "fichas",     label: "Fichas",          done: fichasCount,     total: fichas.length,     color: "#0891b2" },
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[#1e3a5f] mb-1">Mi Progreso</h1>
          <p className="text-slate-500 text-sm">Marca los casos, protocolos y fichas que has revisado.</p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 truncate">{c.diagnostico.split("(")[0].trim()} · {c.totalSesiones} sesiones</p>
                </div>
                <Link
                  href={`/casos/${c.id}`}
      ...
      <p className="text-xs text-slate-400 text-center mt-8">
        El progreso se guarda en este navegador. No se sincroniza entre dispositivos.
      </p>
    </div>
  );
}
